...
DATE_FILE = os.environ.get("TASKS_FILE", "tasks.json")
...
def list_tasks(filter=None):
    tasks = load_tasks()
    filtered_tasks = (
        [task for task in tasks if task["status"] == filter] if filter else tasks
    )
    if not filtered_tasks:
        print("No tasks found.")
        return
    template_text = (
        f"{'ID':<5} {'Status':<8} {'Created':<25} {'Updated':<25} {'Description'}"
    )
    print(f"{template_text} \n{'-'*len(template_text)}")
    for task in filtered_tasks:
        print(
            f"{task['id']:<5} {task['status']:<8} {task['created']:<25} {task['updated']:<25} {task['description']}"
        )
